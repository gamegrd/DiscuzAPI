...
class DiscuzAPI:
	...
	def login(self):
		...
		if self.userName.encode('utf8') in content:
			self.isLogon = True
			print 'logon success!'
			self.initFormhashXq()
			return 1
	...
	def reply(self, tid, subject = u'',msg = u'支持~~~顶一下下~~嘻嘻'):
		...
		postData = urllib.urlencode({'formhash': self.formhash, 'message': msg.encode('utf8'), 'subject': subject.encode('utf8'), 'posttime':int(time.time()) })
		req = urllib2.Request(url,postData)
	...
	def publish(self, fid, subject = u'发个帖子测试一下下，嘻嘻~~~',msg = u'发个帖子测试一下下，嘻嘻~~~', imgId = ""):
		''' 发帖 '''
	...
	def publishPortal(self,catid = 1,subject=u"测试新闻",msg="测试消息"):
		''' 发帖 '''
		url = self.forumUrl + '/portal.php?mod=portalcp&ac=article&catid=%d' % catid
		print(url)	
		postData = urllib.urlencode(
					{	'title': subject.encode('utf8'),
						'highlight_style[0]': 	'',
						'highlight_style[1]': 	'',
						'highlight_style[2]': 	'',
						'highlight_style[3]': 	'',
						...
						'author': 	'',
						'conver': 	'',
						'newalbum': "".join(u'请输入相册名称').encode('utf8'),
		...
		content = urllib2.urlopen(req).read().decode('utf8')
		#print content
		if u"发布文章成功" in content:
			print 'publishPortal success!'
			return 1
		else:
			print 'publishPortal faild!'
			return 0
	...
	def sign(self,msg = u'哈哈，我来签到了！'):
		...
		if self.isLogon and self.xq:
			...
			postData = urllib.urlencode({'fastreply': '1', 'formhash': self.formhash, 'qdmode': '1', 'qdxq': self.xq, 'todaysay':msg.encode('utf8') })
			req = urllib2.Request(url,postData)
			content = urllib2.urlopen(req).read().decode('utf8')
			#print content
	...
	def speak(self,msg = u'hah,哈哈，测试一下！'):
		...
		postData = urllib.urlencode({'addsubmit': '1', 'formhash': self.formhash, 'referer': 'home.php', 'spacenote': 'true', 'message':msg.encode('utf8') })
		req = urllib2.Request(url,postData)
		content = urllib2.urlopen(req).read().encode('utf8')
		#print content
	...
	def uploadPortalImage(self, imageData, catid=1,Filename="test.jpg"):
		imageId = None
		# get the uid and hash
		url = self.forumUrl + "/portal.php?mod=portalcp&ac=article&catid=%d" % catid
		data = urllib2.urlopen(url).read().decode('utf8')
		
		hashReg = re.compile(r'"uid" : "(\d+)", "hash":"(\w+)"', re.S)
		hashRet = hashReg.search( data ).group(1,2)
		uid = hashRet[0]
		hashRet=hashRet[1]
		
		# Upload the image

		uploadImageUrl = self.forumUrl + "/misc.php?mod=swfupload&operation=portal&action=swfupload"
		refer = url
		...
		L.append('--' + BOUNDARY)
		L.append("Content-Disposition: form-data; name=\"catid\""  )
		L.append("")
		L.append(catid)
		...
		#print(body)
		hashReg = re.compile(r'"aid":(?P<aid>\d+).+"smallimg":"(?P<smallimg>.+)", "bigimg":"(?P<bigimg>.+)", "errorcode"', re.S)
		hashRet = hashReg.search( body ).groupdict()
		if hashRet:
			return hashRet 
		else:
			return None
	
	def uploadForumImage(self, imageData, fid=1):
		imageId = None
		# get the uid and hash
		...
		# Upload the image
		uploadImageUrl = self.forumUrl + "/misc.php?mod=swfupload&operation=upload&type=image"
...
if __name__ == '__main__':
	...
	#robot.publish(21,36)
	#robot.reply(10)
	imageData = open('D:\\FRAMES_X8.jpg', 'rb').read()
